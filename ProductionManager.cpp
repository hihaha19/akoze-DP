...
void ProductionManager::update()
{
    ...
    // po uplynuti aspon 3 minut sa kazdych 20 sekund kontroluje, ci mame dost supply depot na vyrabanie jednotiek
    if ((BWAPI::Broodwar->getFrameCount() > 4285) && (BWAPI::Broodwar->getFrameCount() % 480 == 0) && (BWAPI::Broodwar->self()->supplyUsed() + 4 > BWAPI::Broodwar->self()->supplyTotal()))
    {
        if (Config::Debug::DrawBuildOrderSearchInfo)
        {
            BWAPI::Broodwar->printf("Supply deadlock detected, building supply!");
    ...
    if (Config::Strategy::StrategyName == "Terran_0") {

        // ak do 3:45 nemame factory
        if (BWAPI::Broodwar->getFrameCount() > 5357 && UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Factory) == 0)
            m_queue.queueAsHighestPriority(MetaType(BWAPI::UnitTypes::Terran_Factory), true);


        // medzi 4 a 6:00 zacni stavat 2 factory, nech na zaciatku siedmej minuty uz mame 2 postavene
        if (BWAPI::Broodwar->getFrameCount() > 7143 && BWAPI::Broodwar->getFrameCount() < 8572 && (BWAPI::Broodwar->getFrameCount() % 1429 == 0) && UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Factory) < 2)
        {
            if (Config::Debug::DrawBuildOrderSearchInfo)
            {
                BWAPI::Broodwar->printf("malo factory");
            }
            m_queue.queueAsHighestPriority(MetaType(BWAPI::UnitTypes::Terran_Factory), true);
        }

        // pred desiatou minutou maj uz 4 factory, preto medzi 7 a 9 maj uz 4
        if (BWAPI::Broodwar->getFrameCount() > 10000 && BWAPI::Broodwar->getFrameCount() < 12857 && (BWAPI::Broodwar->getFrameCount() % 1429 == 0) && UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Factory) < 4)
        {
            if (Config::Debug::DrawBuildOrderSearchInfo)
            {
                BWAPI::Broodwar->printf("malo factory");
        ...
        // do 13tej minuty maj 5 factory
        if (BWAPI::Broodwar->getFrameCount() > 15715 && BWAPI::Broodwar->getFrameCount() < 18572 && (BWAPI::Broodwar->getFrameCount() % 1429 == 0) && UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Factory) < 5)
        {
            if (Config::Debug::DrawBuildOrderSearchInfo)
            {
                BWAPI::Broodwar->printf("malo factory");
        ...
        if (BWAPI::Broodwar->getFrameCount() > 18572 && (BWAPI::Broodwar->getFrameCount() % 1429 == 0) && UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Factory) < 6)
        {
            if (Config::Debug::DrawBuildOrderSearchInfo)
            {
                BWAPI::Broodwar->printf("malo factory");
        ...
        if (BWAPI::Broodwar->getFrameCount() > 12857 && (BWAPI::Broodwar->getFrameCount() % 1429 == 0) && UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Factory) > 1 && 
            UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Machine_Shop) < 2)
        {
            if (Config::Debug::DrawBuildOrderSearchInfo)
            {
                BWAPI::Broodwar->printf("malo machine shop v production");
            }
            m_queue.queueAsHighestPriority(MetaType(BWAPI::UnitTypes::Terran_Machine_Shop), true);
        }


        // kazdych 20 sekund kontroluj ci mame 2 tanky a 2 vulture
        int numTanks = UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Siege_Tank_Siege_Mode) + UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Siege_Tank_Tank_Mode);
        int numVultures = UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Vulture);

       /*if (BWAPI::Broodwar->getFrameCount() > 8572 && (BWAPI::Broodwar->getFrameCount() % 476 == 0)) {
          if (numTanks < 2)
            {
                if (Config::Debug::DrawBuildOrderSearchInfo)
                {
                    BWAPI::Broodwar->printf("I have only few tanks");
                }
          ...
          if (numVultures < 2)
          {
              if (Config::Debug::DrawBuildOrderSearchInfo)
              {
                  BWAPI::Broodwar->printf("I have only few vultures");
              }
    ...
    for (auto& unit : BWAPI::Broodwar->self()->getUnits())
    {
        ...
        if (BWAPI::Broodwar->self()->getRace() == BWAPI::Races::Terran) {
            if (unit->isCompleted()) {
                if (unit->getType() == BWAPI::UnitTypes::Terran_Factory && unit->getAddon() && (unit->getAddon()->isCompleted()) && (unit->getAddon()->getType() == BWAPI::UnitTypes::Terran_Machine_Shop))
                    if (!BWAPI::Broodwar->self()->hasResearched(BWAPI::TechTypes::Spider_Mines) && (BWAPI::Broodwar->self()->minerals() > 300)) {
                        unit->getAddon()->research(BWAPI::TechTypes::Spider_Mines);
                    }
            }
          ...
          /*if (unit->getType() == BWAPI::UnitTypes::Terran_Vulture && unit->isCompleted()) {
                if (unit->getSpiderMineCount() > 0) {
                    BWAPI::Broodwar->printf("Davam minu\n");
...
void ProductionManager::manageBuildOrderQueue()
{
    ...
    while (!m_queue.isEmpty())
    {
        ...
        if (Config::Strategy::StrategyName == "Terran_0") {
            if (currentItem.metaType.getName() == "Terran_Factory") {
                if (UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Factory) > 5) {
                    m_queue.removeCurrentHighestPriorityItem();
                    break;
                }
            }

            if (currentItem.metaType.getName() == "Terran_Armory") {
                if (UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Armory) > 0) {
                    m_queue.removeCurrentHighestPriorityItem();
                    break;
                }
            }

            if (currentItem.metaType.getName() == "Terran_Starport") {
                if (UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Starport) > 0) {
                    m_queue.removeCurrentHighestPriorityItem();
                    break;
                }
            }

            if (currentItem.metaType.getName() == "Terran_Comsat_Station") {
                if (UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Comsat_Station) > 1) {
                    m_queue.removeCurrentHighestPriorityItem();
                    break;
                }
            }

            if (currentItem.metaType.getName() == "Terran_Machine_Shop") {
                int minute = BWAPI::Broodwar->getFrameCount() / (23.81 * 60);
                if (minute > 7 && minute <= 10 && (UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Machine_Shop) >= 2)) {
                    m_queue.removeCurrentHighestPriorityItem();
                }

                if (minute > 10 && (UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Machine_Shop) >= 3)) {
                    m_queue.removeCurrentHighestPriorityItem();
                }
            }

            int minute = BWAPI::Broodwar->getFrameCount() / (23.81 * 60);
            if (currentItem.metaType.getName() == "Terran_Machine_Shop" && (minute > 7 && minute <= 10 && (UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Machine_Shop) >= 2)) ||
                (minute > 10 && (UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Machine_Shop) >= 3))) {
            //    m_queue.removeSpecificItem(MetaType(BWAPI::UnitTypes::Terran_Machine_Shop));
            }


            

         if (currentItem.metaType.getName() == "Terran_Control_Tower" && (UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Control_Tower)) > 0) {
             m_queue.removeCurrentHighestPriorityItem();
             break;
         }

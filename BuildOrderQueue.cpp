...
// mnou pridana 
void BuildOrderQueue::removeSpecificItem(MetaType metaType) {
    for (auto it = queue.begin(); it != queue.end(); ) {
        printf("Nazov je %s\n", it->metaType.getName().c_str());
        if (it->metaType.getName() == metaType.getName()) {
            it = queue.erase(it);
        }
        else {
            ++it;
...
void BuildOrderQueue::queueAsLowestPriority(MetaType m, bool blocking)
{
    ...
    printf("BuildOrderItem: %s, Priority: %d, Blocking: %s\n",
        m.getName().c_str(),  // 
        newPriority,
        blocking ? "yes" : "no"); // 

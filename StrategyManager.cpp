...
bool staviamMachine3minuta = false;
bool staviamMachine7minuta = false;
bool staviamMachine10minuta = false;
...
bool isBuildingConstructed(BWAPI::UnitType buildingType) {
    for (auto &u : BWAPI::Broodwar->self()->getUnits()) {
        if (u->getType() == buildingType && !u->isCompleted()) {
            return true;
        }
    }
    return false;
...
const MetaPairVector StrategyManager::getTerranBuildOrderGoal() const
{
    ...
    int numComsatStation = UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Comsat_Station);
    int numMachineShop = UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Machine_Shop);
    int numControlTower = UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Terran_Control_Tower);

    if (Config::Strategy::StrategyName == "Terran_0")
    {   
        int frame = BWAPI::Broodwar->getFrameCount();
        int minute = frame / (23.81 * 60);
        int sekunda = frame / 23.81;

        if (minute >= 3 && numMachineShop == 0 && staviamMachine3minuta == false) {
            printf("Nad 3 - skontroluj machine shop\n");
            if (!isBuildingConstructed(BWAPI::UnitTypes::Terran_Machine_Shop) && numMachineShop < 1) {
                goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Machine_Shop, 1));
                printf("Pridavam machine nad 3 minuty\n");
                  staviamMachine3minuta = true;
            }
        }
        

        if (minute >= 4 && minute <= 7) {
            int desiredFactoryCount = 2;
            int factoriesToBuild = desiredFactoryCount - numFactory;

            if (factoriesToBuild > 0) {
                for (int i = 0; i < factoriesToBuild; ++i) {
            ...
            if (true) {
                ...
                printf("posielam vulture\n");
                printf("Cas zapasu %d:%d\n", minute, sekunda%60);
            }
            

            //pridat time stamp
         //   printf("Frame v zapase %d\n ", frame);
        }
            

        if (minute > 7 && minute <= 10) {
            printf("Medzi 7 a 10\n");
            int desiredFactoryCount = 4;
            int factoriesToBuild = desiredFactoryCount - numFactory;
            ...
            if (numMachineShop < 2 && numFactory > 0 && !isBuildingConstructed(BWAPI::UnitTypes::Terran_Machine_Shop) && staviamMachine7minuta == false) {
                goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Machine_Shop, 1));
                printf("pridavam machine shop, mam ich %d\n", numMachineShop);
                staviamMachine7minuta = true;
            }
        ...
        //    goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Siege_Tank_Tank_Mode, 5));
            goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Vulture, numVultures + 3));
            goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Goliath, 2));
        }


        if(minute >= 7 && numAcademy > 0 && numComsatStation == 0 && minute <= 10)
            goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Comsat_Station, 1));

        // postavenie academy po deviatej minute 
        if (minute > 9 && (BWAPI::Broodwar->self()->minerals() > 200) && numAcademy == 0) {
            if (isBuildingConstructed(BWAPI::UnitTypes::Terran_Academy)) {
            }
            else {
        ...
        if (minute > 10) {
          ...
          if (factoriesToBuild > 0) {
            ...
            if(numCC > 1 && numComsatStation < 2) // ak mame aspon 2 comand centre, postav dalsi comsat station
                goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Comsat_Station, 1));

            goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Missile_Turret, 1));

            if (numStarport > 0 && numControlTower == 0)
                goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Control_Tower, 1));

            if (numMachineShop < 3 && numFactory > 2 && !isBuildingConstructed(BWAPI::UnitTypes::Terran_Machine_Shop) && staviamMachine10minuta == false) {
                goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Machine_Shop, 1));
                staviamMachine10minuta = true;
                printf("pridavam machine shop, mam ich %d\n", numMachineShop);
            }

       //   goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Siege_Tank_Tank_Mode, 9));
          goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Vulture, 6));
          goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Goliath, 2));
        }

        if (minute > 14 && numDropship < 1) {
            goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Dropship, 1));
        }

        if (numStarport > 0 && numWraith == 0) {
            goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Wraith, 1));
        }
    }
